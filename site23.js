...
setInterval(blinker, 1000);
	...
	$(document.body).on('click', '.g_t_inf_chlst' ,function(e){
		...
		var myiframe = '<iframe width="100%" height="100%" frameborder="0" allowfullscreen=""  src="'+base_static+'iframes/'+server_id+'.html" scrolling="no"></iframe>';
	...
	$('.given_dateshow').each(function() {
		...
		switch (true) {
		    ...
		    case (x > 0 ):
		        ...
		        break;
		    case (y > 0 ):
		        $this.parent().find(".stsrt58").html('<div class="blink_txt fximres1">جاري الان</div>');
		        break;
		    default:
		        $this.parent().find(".stsrt58").html('<div class="fximres1">انتهى البرنامج</div>');
		        break;
		}
	});
		...
		switch (true) {
		    ...
		    default:
		      var elemt = $this.parent().find(".stsrt58");
		        ...
		        elemt.prev().html('<i class="fa fa-clock-o"></i> انتهت'); 
		        break;
		}
	});
